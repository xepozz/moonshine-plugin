...
private typealias PagesIndexType = String

class PagesIndex : AbstractIndex<PagesIndexType>() {
    companion object {
        val key = ID.create<String, PagesIndexType>("Moonshine.Pages")
    }
    ...
    override fun getIndexer() = DataIndexer<String, PagesIndexType, FileContent> { inputData ->
        inputData
            .psiFile
            .let { PsiTreeUtil.findChildrenOfType(it, PhpClass::class.java) }
            .filter { it.fqn == MoonshineClasses.PAGE }
            .mapNotNull { it.fqn }
            .associateWith { it }
    }
